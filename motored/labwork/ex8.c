...
int main() {
	...
	//GPIO INIT
	GPIOD_MODER = REP_BITS(GPIOD_MODER, Banc_LED*2, 2, GPIO_MODER_OUT);
	GPIOD_OTYPER &= ~(1<<Banc_LED);
	...
	// printf("compteur: %d \n", TIM4_CNT);
		if(TIM4_triggered){
			printf("triggered");
			TIM4_triggered = 0;
			if((GPIOD_ODR & (1<<Banc_LED)) == 0)
				GPIOD_BSRR = 1 << Banc_LED;
			else
				GPIOD_BSRR = 1 << (Banc_LED + 16);
		}
	}
}
